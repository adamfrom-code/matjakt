...
- **Egna produkthändelser** (knapptryck i appen) skickas till `POST /api/analytics/event` med enbart `{"event": "<namn>"}` - ingen cookie, inget konto-id, ingen plats. Backend räknar bara upp en daglig siffra per händelsenamn (`ANALYTICS_ALLOWED_EVENTS` i `backend/api_server.py`); allt annat avvisas. Anropet är "fire and forget" (`fetch(...).catch(() => {})`) och kan aldrig hindra en knapp från att fungera, även om anropet blockeras av en annonsblockerare eller CORS ännu inte tillåter den nya domänen.
