...
const state = { budget: 800, personer: 2, middagar: 4, butik: "auto", postnummer: "80313", position: null, sokning: "", kategori: "alla", maxTid: 0, baraFavoriter: false, apiRecipes: [], pantry: savedState.pantry || {}, liveValda: savedState.liveValda || [], liveProdukter: [], liveProduktMap: {}, liveLaddar: new Set(), valdaProdukter: savedState.valdaProdukter || {}, antal: savedState.antal || {}, favoriter: new Set(savedState.favoriter || []), plan: savedState.plan || {}, valda: new Set(), avklarade: new Set(), expanded: null };
function saveState() { localStorage.setItem("matjakt-state", JSON.stringify({ pantry: state.pantry, liveValda: state.liveValda, liveProdukter: state.liveProdukter, valdaProdukter: state.valdaProdukter, antal: state.antal, favoriter: [...state.favoriter], plan: state.plan })); }
const BRANCHES = [
...
function loadBasketProductImages(items, store) { if (!["Willys", "Hemköp"].includes(store)) return; const fallback = state.liveProdukter[0]; items.filter(item => !state.liveProduktMap[item.namn] && !state.liveLaddar.has(item.namn)).forEach(async item => { state.liveLaddar.add(item.namn); try { const response = await fetch(`http://127.0.0.1:8000/api/products?butik=${encodeURIComponent(store)}&q=${encodeURIComponent(item.namn)}`); const data = await response.json(); state.liveProduktMap[item.namn] = data.produkter?.[0] || fallback || null; } catch { state.liveProduktMap[item.namn] = fallback || null; } finally { state.liveLaddar.delete(item.namn); renderBasket(); applyLiveImages(); } }); }
...
function step(key, delta, min, max) { state[key] = Math.min(max, Math.max(min, state[key] + delta)); $(`${key === "personer" ? "people" : "meals"}Value`).textContent = state[key]; render(); }
$("budgetInput").addEventListener("input", e => { state.budget = Number(e.target.value) || 0; renderBasket(); });
$("postcodeInput").addEventListener("input", e => { state.position = null; state.postnummer = e.target.value.replace(/\D/g, ""); chooseMenu(false); });
