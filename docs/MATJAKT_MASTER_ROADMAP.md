...
Finns: E0 render-buss, E9 (butiksvalet utan kombinatorik, budgetfältet triggar inget omval), D7 streaming, D8 `busy_timeout` + schema en gång per process + självläkande cache, esbuild-bundle (~141 kB rå), `CANDIDATE_POOL_FOR_COUNT`, livepriser 5/20 per anrop med 429-stopp, `scripts/acceptance_speed.py` (flödet under pågående import), U03 mätt 1,7–5,3 s till första prissatta lista (`test_e2e_matt.py`). **Baslinje 2026-09-16 (lokalt, varm, mot backend :8000):** DOMContentLoaded 421 ms, load 452 ms, 73 resurser, 1,36 MB; boot hämtar `/api/recipes?limit=500` (260 KB) + `shelves` (130 KB); Veckan ritas på 15 ms. Vid boot gick `entitlements` ×2 och `shelves` ×2 — rättade i AM1 (en spärr för boot/inloggning/uppvaknande) och AM2 (in-flight-vakt för hyllorna). Saknas: ingen prestandagrind i CI (U03-testet faller först över 30 s), bilder utan `width/height` (E15-notering), ingen mätning på riktig telefon (U79), hela receptbanken (260 KB) hämtas vid varje start utan cache. Restarbete: sänk taket i `test_e2e_matt.py` till en uppmätt nivå × 2, Lighthouse-körning mot bygget i `deploy.yml`.
