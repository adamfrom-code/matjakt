"""
ICA.se prisscraper (Playwright)
================================
Se README.md i projektroten för bakgrund och kända begränsningar.
...
def valj_butik(page, postnummer: str):
    """Simulerar butiksval via postnummer.
    OBS: selektorerna nedan är GISSNINGAR - öppna handla.ica.se i Chrome,
    tryck F12 och inspektera "Välj butik"-flödet, justera sedan här."""
    page.goto("https://handla.ica.se/", wait_until="networkidle")
...
def main():
    produkter = [p for p in las_produktlista(PRODUKTFIL) if p.get("ica_url")]
    resultat = []
    ...
    with sync_playwright() as p:
        ...
        for produkt in produkter:
            rad = hamta_pris(page, produkt["ica_url"], produkt["namn"])
    ...
    if resultat:
        skriv_csv(resultat, "resultat_ica.csv")
        skriv_json(resultat, "resultat_ica.json")
    else:
        print("Inga produkter med ica_url hittades i produktfilen.")
